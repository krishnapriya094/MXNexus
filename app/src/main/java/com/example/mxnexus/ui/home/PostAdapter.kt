...
class PostAdapter(
    ...
    private val currentUserRole: String, // Added role to check for Admin
    private val onLikeClick: (Post) -> Unit,
    private val onCommentClick: (Post) -> Unit,
    private val onShareClick: (Post) -> Unit,
    private val onDeleteClick: (Post) -> Unit,
    private val onFollowClick: ((Post) -> Unit)? = null,
    ...
    class PostViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val tvScheduledLabel: TextView  = itemView.findViewById(R.id.tvScheduledLabel)
        ...
        val btnShare: LinearLayout      = itemView.findViewById(R.id.btnPostShare)
        val btnMore: ImageView          = itemView.findViewById(R.id.btnPostMore)
        val btnSave: ImageView          = itemView.findViewById(R.id.btnPostSave)
    ...
    override fun onBindViewHolder(holder: PostViewHolder, position: Int) {
        ...
        if (post.userId == currentUserId) {
            ...
            holder.btnFollow.setOnClickListener { onFollowClick?.invoke(post) }
        }

        // Avatar Loading
        if (post.profileImageUrl.isNotBlank()) {
            Glide.with(ctx).load(post.profileImageUrl).apply(RequestOptions.circleCropTransform()).placeholder(R.drawable.ic_profile).into(holder.imgAvatar)
        } else {
        ...
        if (post.imageUrl.isNotBlank()) {
            ...
            Glide.with(ctx).load(post.imageUrl).apply(RequestOptions().transform(RoundedCorners(16))).into(holder.imgPostPhoto)
        ...
        holder.icLike.setImageResource(if (isLiked) R.drawable.ic_heart_filled else R.drawable.ic_heart_outline)
        ...
        holder.btnSave.setOnClickListener {
            Toast.makeText(holder.itemView.context, "Saved to your library!", Toast.LENGTH_SHORT).show()
        }

        // ADMIN POWER: Show delete for OWN posts OR if user is ADMIN
